...
def build_dict(imgs, wtoi, params):
  ...
  for img in imgs:
    if (params['split'] == img['split']) or \
      ...
      ref_idxs = []
      tmp_paragraph = ''
      sents = img['sentences']['tokens']
      for i,sent in enumerate(sents):
        if i==0:
          tmp_paragraph = tmp_paragraph + sent + ' <EOS>'
        else:
          tmp_paragraph = tmp_paragraph +' ' +sent + ' <EOS>'
      tmp_tokens = tmp_paragraph.split(' ')
...
if __name__ == "__main__":
  ...
  # input json
  parser.add_argument('--input_json', default='../data/para_paragraph_captions_dense.json', help='input json file to process into hdf5')
  parser.add_argument('--dict_json', default='../data/paratalk.json', help='output json file')
  parser.add_argument('--output_pkl', default='../data/para-train', help='output pickle file')

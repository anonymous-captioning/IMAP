...
cmd:option('-checkpoint',
  'data/models/densecap-pretrained-vgg16.t7')
...
cmd:option('-input_txt', 'imgs_all_path.txt')
cmd:option('-max_images', 0)
cmd:option('-output_h5', 'VG_feature.h5')
...
local function run_image(model, img_path, opt, dtype)
  ...
  local boxes_xcycwh, feats,lstm_input = model:extractFeatures(img_caffe:type(dtype))
  local boxes_xywh = box_utils.xcycwh_to_xywh(boxes_xcycwh)
  return boxes_xywh, feats,lstm_input
end
...
local function main()
  ...
  local all_boxes = torch.FloatTensor(M, 4):zero()
  local all_feats = torch.FloatTensor(M, D):zero()
  ...
  for i, image_path in ipairs(image_paths) do
    ...
    local boxes, feats, lstm_input = run_image(model, image_path, opt, dtype)
    all_boxes:copy(boxes[{{1, M}}])

    all_feats:copy(feats[{{1, M}}])
    print(image_path)
    local file={}
    local name = {}
    for w in string.gmatch(image_path,"([^'/']+)") do
        table.insert(file,w)
    end
    for w in string.gmatch(file[#file],"([^'.']+)") do
        table.insert(name,w)
    end
    local file_name = name[#name-1]
    h5_file:write('/feats/'..file_name, all_feats)
    h5_file:write('/boxes'..file_name, all_boxes)
